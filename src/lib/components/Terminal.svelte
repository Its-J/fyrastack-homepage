<script lang="ts">
	...
	$effect(() => {
		void lines.length; void currentText; void inlinePrefix;
		tick().then(() => { if (termEl) termEl.scrollTop = termEl.scrollHeight; });
	});
	...
	function out(text: string, cls: LineCls = 'output') { lines.push({ kind: 'output', text, cls }); }
	function blank() { lines.push({ kind: 'blank' }); }
	function clear() { lines = []; currentText = ''; }

	onMount(() => {
		const cursor = setInterval(() => { showCursor = !showCursor; }, 530);
		animate();
	...
	async function animate() {
		while (true) {
			...
			out("The authenticity of host 'msp1.fyrastack.com (45.61.185.22)' can't be established.", 'system');
			...
			out("Warning: Permanently added 'msp1.fyrastack.com' (ED25519) to the list of known hosts.", 'dim');
			await sleep(700);
			...
			await sleep(220);
			out('Linux msp1 6.11.8-300.fc41.x86_64 #1 SMP PREEMPT_DYNAMIC Thu Nov 14 18:02:31 UTC 2025 x86_64 GNU/Linux');
			blank();
			await sleep(1000);
			...
			await sleep(220);
			out('               total        used        free      shared  buff/cache   available', 'dim');
			...
			out('     Loaded: loaded (/usr/lib/systemd/system/caddy.service; enabled; preset: enabled)', 'dim');
			out('     Active: active (running) since Wed 2026-01-14 10:08:03 CST; 47 days ago', 'success');
			out('    Process: 1241 ExecStartPre=/usr/bin/caddy validate --config /etc/caddy/Caddyfile', 'dim');
...
<section class="border-b border-fyra-gray-800">
	<div class="bg-fyra-gray-900">
		<div
			bind:this={termEl}
			class="h-96 overflow-y-auto px-5 py-4 font-mono text-sm leading-relaxed scrollbar-none"
		>
			{#each lines as line}
				{#if line.kind === 'blank'}
					<div class="h-3"></div>
				{:else if line.kind === 'cmd'}
					<div class="flex flex-wrap">
						{#if line.prompt === 'local'}
							<span class="select-none text-fyra-gray-600">$&nbsp;</span>
						{:else}
							<span class="select-none text-fyra-gray-600">deploy@</span><span class="select-none text-fyra-red-500">msp1</span><span class="select-none text-fyra-gray-700">:~$&nbsp;</span>
						{/if}
			...
			<div class="flex items-center">
				...
				{:else if currentPrompt === 'remote'}
					<span class="select-none text-fyra-gray-600">deploy@</span><span class="select-none text-fyra-red-500">msp1</span><span class="select-none text-fyra-gray-700">:~$&nbsp;</span>
				{:else}
					<span class="select-none text-fyra-gray-500">{inlinePrefix.trimEnd()}&nbsp;</span>
				{/if}
				<span class="text-fyra-gray-100">{currentText}</span>
				<span class="ml-px inline-block h-[1em] w-[0.5em] translate-y-[0.05em] {showCursor ? 'bg-fyra-gray-400' : 'bg-transparent'}"></span>
			</div>
		</div>
	</div>
</section>
